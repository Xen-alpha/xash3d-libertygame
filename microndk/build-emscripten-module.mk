...
#	echo $(OBJ_FILES) $(INCLUDES)
	$(CXX) -s UNALIGNED_ACCESS_FUNCTION_LIST=\"$(MICRONDK_DIR)unaligned.txt\" -O3 --js-opts 1 --llvm-lto 3 -g0 -s INLINING_LIMIT=10 -s ELIMINATE_DUPLICATE_FUNCTIONS=1 -s AGGRESSIVE_VARIABLE_ELIMINATION=1 -s SIDE_MODULE=1 -s WEBSOCKET_URL=\'wss://liberty.banipest.com/$(LOCAL_MODULE)\' -s SHELL_FILE=\"$(MICRONDK_DIR)shell_fakedynamiclib.js\" -s -static-libgcc -static-libstdc++ -o ../$(MODULE_FILE) $(ARCH_LIBS) $(LDFLAGS) $(OBJ_FILES) $(ARCH_LIBS) $(LIBS) $(LOCAL_LDFLAGS) $(LOCAL_LDLIBS) -Wl,--no-warn-mismatch -Wl,--no-undefined
clean:
	rm -f $(OBJ_FILES)
